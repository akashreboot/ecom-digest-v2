...
> "Five things in order of leverage. First, pytest invariants for the ranker. Second, calendar awareness — suppress Jan 1, Holi, etc. Third, engagement feedback loop — thumbs up/down on findings + click tracking. Fourth, peer benchmarks — 'brands like yours typically see X this time of year' with k-anonymous aggregation. Fifth, structured causal-confidence scoring — let the UI surface high-confidence findings differently from speculative ones."
